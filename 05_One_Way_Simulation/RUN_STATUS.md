...
> **⚠ 交接**：接手者先讀 **`../HANDOFF_CURRENT.md`**（現況/判準/鐵則/待裁決事項）。
> 接手者只監控回報，**所有決策給 Wade 裁決**。本檔為完整戰史，交接文件為行動指南。

## ★★ 一頁現況摘要（2026-07-12，v6 里程碑＝現行定版）

**三尺度單向鏈全部完成、v6 重跑定版、下游圖表與論文數字全部更新（詳 ../HANDOFF_CURRENT.md）。**

- **問題1修正**：s1 門檻過廣 → 大/小模 s1 以 **T=1.0** 重跑（診斷=diag_T1_sweep.f3dat 掃描 T=0.80–1.00；
  大模活化 117,621→3,315、小模 422,854→24,935）；s2-11 驅動檔接縫平移保留增量
  （gen_s1_reruns.py＋large/small_s1_rerun.f3dat；舊檔備份 _backup_T08run/）
- **問題2修正**：拱腳錨定帶隨 3.74% 縱坡（z≤rim(y)+0.41）＋6階退火釋放舊帶
  （couple_staged_v6.f3dat；CONTROL-0=28 裂縫＝v5 基準；圖5-19 三角形假象消除）
- **v6 增量序列**：s1=2,751（基準）→ 升水 3,021/5,298/6,768/2,553 → **s6=14,570（濕峰）**
  → 退水 1,509/553/480/389 → **s11=88（凍結，2.9 條/日）**；全程零 Illegal
- **定版指標（07-12 Wade 核定新定義）**：A_wet＝r_wet(s2-6)/r_dry1(s1)＝**7.0**；
  A_frz＝r_dry2(s11)/r_wet＝**0.0046**；總損傷密度 1.83%（水循環分量 1.70%）；
  裂縫 環199/斜19/縱3 m；左肩推力峰 1,029 kN/m；兩腰內擠均值 0.15 mm；外壓峰 946 kPa
- **產物**：cs_01/06/11.f3sav＋cs_s1-11 cracks/cwall/pmap＋cp_cforce/balldisp/pf 三檢查點
  ＋lg/sm s01 場量射線全部重匯；圖5-06~5-19＋表5-1~5-5＝00_Document/result（論文版）；
  論文全文=00_Document/260711_TX碩論_Wade.docx
- **待辦**：Ch5 深度審閱（Codex CLI 協審）→ 全文正式排版（參照蔡承翰碩論定稿格式）

---
## 以下為歷史戰史（07-02 ~ 07-08 v2 跑通紀錄，供追溯）
