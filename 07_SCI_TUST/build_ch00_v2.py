...
# ========================== 02 參考文獻主庫：內部證據台帳 ==========================
d = new_doc()
H(d, "參考文獻主庫與證據台帳（內部工作版；88 篇）")
P(d, "主庫目前為 88 篇（工作庫，非投稿清單），其中 TUST 39 篇（44%）；全文精讀 50 篇。投稿時應自本庫精選 55–65 篇成正式清單，屆時再把 TUST 佔比拉回 50% 以上。這一份是把書目、蒐集分箱與證據可用性放在一起的"
...
TBL(d, ["等級", "數量", "Access／review 狀態", "暫定 citation readiness"], [
    ...
    ["C", "38", "abstract／metadata；含本機 PDF 待讀與新蒐集未精讀者", "只作背景或檢索線索；不得推論章節、圖表及未載數字"],
...
P(d, "分類（v6，Wade 07-25 定義之六類）：**營運隧道案例 → 圍岩依時變形 → 水力耦合分析 → "
     "三維地質模型 → 跨尺度數值方法 → 襯砌受力與損傷**，與六大賣點逐一對位。每子類含判準與"
     "引用位置，一篇只放一個子類；分類指出「該文獻在本文中的角色」，仍不得代替其研究對象、"
     "荷載路徑與材料之實際涵蓋範圍。",
  size=10.5, point="子類＝寫作角色；證據力另依 A/B/C 分級與逐篇筆記判讀")
P(d, "缺口進度（07-25）：**第 4 類三維地質模型已自 0 補至 12 篇**（4A/4B/4C 各 4 篇，"
     "含 IAEG Commission 25 之工程地質模型定義、深埋隧道模型橫向範圍量化判準、I-BIM 地質→"
     "數值端到端轉換、地層界面不確定性）；**子類 3A 近場滲流解析自 4 補至 9 篇**"
     "（含水下圓形隧道穩態滲流解析、含滲流力之地盤反應曲線、水力—力學全耦合應變軟化解）。"
     "全數 Crossref 逐 DOI 驗證、僅收 Q1、零 MDPI 與預印本。", size=10.5,
  point="兩處缺口已補齊；投稿前再依主線精選成正式清單")
