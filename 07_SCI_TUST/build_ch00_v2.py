...
# ========================== 02 參考文獻主庫：內部證據台帳 ==========================
d = new_doc()
H(d, "參考文獻主庫與證據台帳（內部工作版；71 篇）")
P(d, "主庫目前為 71 篇，其中 TUST 32 篇（45%）；全文精讀 50 篇（70%）；另 12 篇取自本機藏書（case／analyze／model）尚待精讀。這一份是把書目、蒐集分箱與證據可用性放在一起的"
...
TBL(d, ["等級", "數量", "Access／review 狀態", "暫定 citation readiness"], [
    ...
    ["C", "21", "abstract／metadata（含 12 篇本機 PDF 待讀）／題名層級", "只作背景或檢索線索；不得推論章節、圖表及未載數字"],
...
P(d, "⚠ 兩處缺口：**第 4 類（三維地質模型）目前 0 篇**——賣點 3 在前言將無文獻可引，"
     "須以「3D engineering geological model／geological modelling with borehole integration／"
     "implicit geological modelling」補 4–6 篇；**子類 3A（近場滲流解析）僅 4 篇**，"
     "建議再補 3–4 篇以撐起水力耦合之驅動端論述。", size=10.5,
  point="4 類全缺、3A 偏薄——為目前兩個必補項")
